...
function Header() {
    ...
    const repositoriesOn = useOnScreen('#repositoriesTitle', '-70px 0px 0px 0px')

    useEffect(() => {
        if (homeOn) setActiveHeaderButton(1)
        else if (projectsOn) setActiveHeaderButton(2)
        else if (contactOn) setActiveHeaderButton(3)
        else if (repositoriesOn) setActiveHeaderButton(4)
    }, [homeOn, projectsOn, contactOn, repositoriesOn])
        ...
        <>
            <div className="my-header">
                <div className="buttons-container">
                    ...
                    <Button text="Repositories" activeNumber={[activeHeaderButton, 4]} onClick={() => scrollTo('repositories')} />
